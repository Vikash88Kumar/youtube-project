...
import { getVideoById } from "../services/video.api";
import { toggleSubscription } from "../services/subscriptions.api";
import { createComment, getVideoComments } from "../services/comments.api";
import { likeVideos, getLikedperVideo } from "../services/likes.api";
...
export default function VideoPlayer() {
  ...
  const handleToggleSubscription = async () => {
    if (subLoading || !owner?._id) return;
      ...
      <div className="flex pt-16">
        ...
        <main className="flex-1 lg:ml-64 px-4 lg:px-8 pb-8 pt-8">
          <div className="max-w-[1800px] mx-auto flex flex-col lg:flex-row gap-6">
            <div className="flex-1">
              {/* VIDEO */}
              <div className="aspect-video bg-black rounded-xl flex justify-center mb-4">
                <video
                  src={video.videoFile}
                  controls
                  poster={video.thumbnail}
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      @{owner.username} • {subscribersCount} subscribers
                    </p>
                  </div>
                </div>
